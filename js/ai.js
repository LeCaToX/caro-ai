...
function makeMove(moveBoard, movePointer, XO){
	currGame.sq[moveBoard.moveRow[movePointer]][moveBoard.moveCol[movePointer]] = XO;
}
function undoMove(moveBoard, movePointer){
...
function AIthink(player, move){ // AI will think for a player.
	if (player == O){
		alphabeta(O, -Infinity, Infinity, ctrl.difficulty);
	} else {
		alphabeta(X, -Infinity, Infinity, ctrl.difficulty);
	}
